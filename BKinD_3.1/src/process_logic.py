...
from data_processing.solve_structure import solve_structure
...
def process_data(output_folder, target_percentages, filtering_percentage, run_refine_wght, run_solve_removed, run_solve_remaining, xds_directory=None, xray=False):
    ...
    def run_tasks():
        ...
        if run_solve_removed:
            solve_structure(output_folder, target_percentages, solve_remaining=False, solve_removed=True, update_progress=update_progress)

        if run_solve_remaining:
            solve_structure(output_folder, target_percentages, solve_remaining=True, solve_removed=False, update_progress=update_progress)
