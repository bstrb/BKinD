...
def process_res_file(fp, data_dict):
    with open(fp, "r") as f:
        ...
        for line in lines:
            if "REM R1 =" in line:
                # Split the line by '=' to get the part after 'REM R1 ='
                after_equal = line.split("=")[1].strip()

                # Extract R1 values from the line
                # The values are usually followed by the word "for", so we can split by "for" and get the numeric parts
                parts = after_equal.split("and")
                
                if len(parts) > 1:
                    # Extract the second R1 value
                    second_r1 = parts[1].split()[0]
                    data_dict['R1'].append(second_r1)
...
def save_refinement_stats(refinement, wdir):
    stats_dir = os.path.join(os.path.dirname(wdir), "REFINEMENT_STATISTICS")
