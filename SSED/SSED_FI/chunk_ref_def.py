...
# def process_run_chunk_folders(base_path, pdb_filename, mtz_orig_filename, output_filename, bins=10):
def process_run_chunk_folders(base_path, pdb_filename, bins=10):
    pattern = os.path.join(base_path, f"merge_chunk_*")
    folder_paths = glob.glob(pattern)  # Use glob to match folders with pattern
    print(f'Found folders: {folder_paths}')
    
    for folder_path in folder_paths:
        # process_folder(folder_path, pdb_filename, mtz_orig_filename, output_filename, bins)
