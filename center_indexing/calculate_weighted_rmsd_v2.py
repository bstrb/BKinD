...
def calculate_weighted_rmsd(fs_ss, intensities, ref_fs_ss, tolerance_factor=2.0):
    ...
    print(f"[DEBUG] len(fs_ss) = {len(fs_ss)}")
    print(f"[DEBUG] len(intensities) = {len(intensities)}")
    print(f"[DEBUG] len(ref_fs_ss) = {len(ref_fs_ss)}")
    ...
    if len(distances) == 0:
        print("Warning: Distances are empty.")
        return float('inf')

    # Handle cases where distances or weights are empty or invalid
    if len(weights) == 0:
        print("Warning: Weights are empty.")
        return float('inf')
