...
def copy_without_dataset(source_group, dest_group, exclude_paths=None):
...
def process_image_worker(args):
    ...
    try:
        ...
        logging.error(f"Error processing image {image_index}: {e}", exc_info=True)
...
def remove_background(h5_file_path, new_h5_file_path, selected_indices=None):
    ...
    with h5py.File(new_h5_file_path, 'w') as new_h5_file:
        ...
        # Create the 'images' dataset in the new file with appropriate chunking
        images_group = new_h5_file['/entry/data']
        corrected_images_dataset = images_group.create_dataset(
            'images',
            shape=(total_images,) + image_shape,
            dtype='float32',
            chunks=(1,) + image_shape  # Set chunk size to 1 image
        )

        # Determine chunk size (adjust based on your memory constraints)
        chunk_size = 1000  # Adjust this number as needed

        # Initialize the multiprocessing pool outside the chunk loop
