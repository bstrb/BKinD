    ...
    try:
        ...
        for level in intensity_levels:
            ...
            if plot:
                ...
                plt.title(f'Contour at level {level:.3f}')
                plt.imshow(image * mask, cmap='gray', origin='lower')
                plt.plot(x_coords, y_coords, 'b.', label='Contour Points')
                circle = plt.Circle((xc, yc), R, color='red', fill=False, linewidth=2, label='Fitted Circle')
                ax = plt.gca()
                ax.add_patch(circle)
                plt.scatter(xc, yc, color='yellow', marker='x', s=100, label='Circle Center')
                plt.legend()
                plt.show()
