...
def process_image(image, image_index, mask, plot=False, verbose=True):
    try:
        ...
        filtered_image = zoom(filtered_image, downsample_factor)
        mask = zoom(mask, downsample_factor, order=0)
        ...
        slice_angle = np.deg2rad(180)  # 160 degrees (±80 degrees)
        num_slices = 4  # Reduced from 8
        ...
        center = result.x / downsample_factor  # Adjust center back to original scale
        if verbose:
            print(f"Computed center at (x={center[0]:.2f}, y={center[1]:.2f})", flush=True)
...
if __name__ == '__main__':
    ...
    selected_indices = [1200]  # Change to desired indices
