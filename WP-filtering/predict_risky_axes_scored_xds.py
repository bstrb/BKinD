...
# python predict_risky_axes_scored_xds.py --xds CORRECT.LP --centering P --uvw-max 5 --thickness-nm 150
...
def classify_lattice_type(a,b,c,al,be,ga):
    # crude metric classification for header only
    eps_len = 1e-3 * max(a,b,c)
    equal_ab = abs(a-b) < eps_len
    equal_bc = abs(b-c) < eps_len
    al90 = abs(al-90.0) < 1e-1
    be90 = abs(be-90.0) < 1e-1
    ga90 = abs(ga-90.0) < 1e-1
    if equal_ab and equal_bc and al90 and be90 and ga90:
        return "cubic"
    if equal_ab and al90 and be90 and ga90:
        return "tetragonal"
    if al90 and be90 and ga90:
        return "orthorhombic"
    if be90 and (not al90 or not ga90):
        return "monoclinic"
    return "triclinic"

def parse_xds_headers(path: str):
    txt = _read_text(path)

    # Unit cell
    m = re.search(r'^\s*UNIT_CELL_CONSTANTS=\s*' +
                _FLOAT_RE + r'\s+' + _FLOAT_RE + r'\s+' + _FLOAT_RE + r'\s+' +
                _FLOAT_RE + r'\s+' + _FLOAT_RE + r'\s+' + _FLOAT_RE,
                txt, re.MULTILINE)
    if not m:
        fail("Could not find UNIT_CELL_CONSTANTS= line in XDS file")
    a,b,c,al,be,ga = map(float, m.groups())

    # Space group number (for info)
    m_sg = re.search(r'^\s*SPACE_GROUP_NUMBER=\s*(\d+)', txt, re.MULTILINE)
    sgnum = int(m_sg.group(1)) if m_sg else None

    # Geometry: NX,NY,QX,QY
    m_geom = re.search(r'^\s*NX=\s*(\d+)\s+NY=\s*(\d+)\s+QX=\s*' +
                    _FLOAT_RE + r'\s+QY=\s*' + _FLOAT_RE,
                    txt, re.MULTILINE)
    if not m_geom:
        fail("Could not find NX= NY= QX= QY= line in XDS file")
    NX = int(m_geom.group(1)); NY = int(m_geom.group(2))
    QX_mm = float(m_geom.group(3)); QY_mm = float(m_geom.group(4))

    # Detector distance
    m_dist = re.search(r'^\s*DETECTOR_DISTANCE=\s*' + _FLOAT_RE, txt, re.MULTILINE)
    if not m_dist:
        fail("Could not find DETECTOR_DISTANCE= in XDS file")
    dist_mm = float(m_dist.group(1))

    # Wavelength
    m_wl = re.search(r'^\s*X-RAY_WAVELENGTH=\s*' + _FLOAT_RE, txt, re.MULTILINE)
    if not m_wl:
        fail("Could not find X-RAY_WAVELENGTH= in XDS file")
    wavelength_A = float(m_wl.group(1))

    # Derived "CrystFEL-like" geometry
    pixel_size_m = (QX_mm * 1e-3)  # assume square pixels, use QX
    if pixel_size_m <= 0:
        fail(f"Non-positive pixel size from QX={QX_mm}")
    res_px_per_m = 1.0 / pixel_size_m
    clen_m = dist_mm * 1e-3

    lattice_type = classify_lattice_type(a,b,c,al,be,ga)
...
def canonical_uvw(u,v,w):
    ...
    if g == 0:
        return None
    u, v, w = u//g, v//g, w//g
    # Canonical: make "forward" direction
...
def _worker_init(Astar, nx, ny, clen_m, wavelength_A, res_px_per_m,
    ...
    _G["use_S"] = use_S
    _G["Smax"] = Smax
    _G["k_wave"] = 1.0 / max(1e-12, wavelength_A)
    _G["g0"] = g0
    _G["p"] = p
...
def main():
    ap = argparse.ArgumentParser(description="Predict problematic ZOLZ axes (CRED/XDS geometry).")
    ap.add_argument("--xds", required=True, help="Path to XDS.INP or CORRECT.LP with geometry + cell")
    ...
    ap.add_argument("--thickness-nm", type=float, default=None,
                    help="Crystal thickness in nm (S_max ≈ 1/(2t) with t in Å)")
    ...
    ap.add_argument("--centering", type=str, default="P",
                    help="Lattice centering symbol (P,A,B,C,I,F,R). Default: P.")
    args = ap.parse_args()

    try:
        cell, geom = parse_xds_headers(args.xds)
    except Exception as ex:
        fail(str(ex))

    centering = (args.centering or "P").upper()

    A, Astar = cell_matrices(cell["a"], cell["b"], cell["c"],
                             cell["al"], cell["be"], cell["ga"])
    ...
    print(f"Cell: {cell['lattice_type'].upper()} {centering} | "
          ...
          f"(SG={cell['space_group_number']})")
    print(f"Geom (from XDS): λ={geom['wavelength_A']} Å, L={geom['clen_m']} m "
          f"(dist={geom['dist_mm']} mm), res={geom['res_px_per_m']} px/m | "
          f"panel {nx}×{ny}px (QX={geom['QX_mm']} mm, QY={geom['QY_mm']} mm) | "
          f"edge r={r_edge_px:.2f}px")
    if use_S:
    ...
    with ctx.Pool(processes=os.cpu_count(),
                  ...
                  initargs=(Astar, nx, ny, geom["clen_m"], geom["wavelength_A"],
                            ...
                            use_S, Smax, args.g0, args.p)) as pool:
    ...
    base = os.path.splitext(os.path.basename(args.xds))[0]
    out_csv = base + "_problematic_axes_scored.csv"
    with open(out_csv, "w", newline="") as f:
        f.write("# CRED/XDS problematic zone axes prediction (centering + S-gate + low-order weight + area norm)\n")
        f.write(f"# XDS file: {args.xds}\n")
        f.write(f"# Cell: {cell['lattice_type'].upper()} {centering} | "
                f"a={cell['a']:.4f} Å b={cell['b']:.4f} Å c={cell['c']:.4f} Å "
                f"al={cell['al']:.2f}° be={cell['be']:.2f}° ga={cell['ga']:.2f}° "
                f"(SG={cell['space_group_number']})\n")
        f.write(f"# Geom (from XDS): λ={geom['wavelength_A']} Å, L={geom['clen_m']} m "
                f"(dist={geom['dist_mm']} mm), res={geom['res_px_per_m']} px/m | "
                f"panel {nx}×{ny}px (QX={geom['QX_mm']} mm, QY={geom['QY_mm']} mm) | "
                f"edge r={r_edge_px:.2f}px\n")
        if use_S:
