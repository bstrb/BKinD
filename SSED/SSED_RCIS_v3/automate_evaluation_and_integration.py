...
from emp_int_def import fast_integration
from generate_bash_script import generate_bash_script 
from emp_merge_def import merge_and_write_mtz
from emp_ref_def import process_run_folders
...
def automate_evaluation_and_integration(stream_file_folder, exp_list, lattice, ring_size, cellfile_path, pointgroup, num_threads, bins, pdb_file, min_res = 2, iterations = 3):
    for exp in exp_list:
        try:
            ...
            best_results_sol = f"{stream_file_folder}/best_results_{EMP}.sol"
            ...
            bash_file_name = f"{EMP}"
            bash_file_path = os.path.join(stream_file_folder, bash_file_name) + ".sh"
            output_stream_format = f"{stream_file_folder}/{EMP}/int.stream"
            ...
            integration_output_folder = f"{stream_file_folder}/{EMP}"
