...
def generate_bash_script(bash_file_name, stream_files_dir,
                        integration_output_name, integration_output_dir, 
                        num_threads ='7', geom_file=None, lst_file=None, 
...
indexamajig -i {lst_file_path}  \\
-g {geom_file_path}  \\
-p {cell_file_path}  \\
-j {num_threads}  \\
-o {integration_output_dir}/{integration_output_name}.stream   \\
--indexing=file --fromfile-input-file={input_sol_file}   \\
--no-revalidate --no-retry --integration=rings --no-refine \\
--no-half-pixel-shift --no-check-cell \\
