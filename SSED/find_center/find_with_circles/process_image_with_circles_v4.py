...
def process_image(image, image_index, mask=None, plot=True, verbose=True):
    ...
    try:
        ...
        # Exclude diffraction spots by setting an upper intensity threshold
        max_intensity_threshold = 0.9  # Adjusted threshold
        norm_image[norm_image >= max_intensity_threshold] = np.nan

        # Exclude diffraction spots by setting a lower intensity threshold
        min_intensity_threshold = 0.1  # Adjusted threshold
        norm_image[norm_image <= min_intensity_threshold] = np.nan

        # Use nan* functions to calculate statistics while ignoring NaNs
        median = np.nanmedian(norm_image)
        mean = np.nanmean(norm_image)
        std = np.nanstd(norm_image)

        print(f"median = {median}, mean = {mean}, std = {std}")

        # Define binning intensities and widths in percentage of maximum intensity
        bin_size_percent = 0.01/1000  # Adjusted bin size in percentage
        # Use numpy.arange for floating point steps
        intensity_levels_percent = np.arange(mean, mean + 3*std, bin_size_percent * 2)
        bin_widths_percent = [bin_size_percent] * len(intensity_levels_percent)
        ...
        for intensity_level_percent, bin_width_percent in zip(intensity_levels_percent, bin_widths_percent):
            ...
            if len(indices) == 0:
                if verbose:
                    print(f"No pixels found in intensity bin {intensity_level_percent}% ± {bin_width_percent / 2}%", flush=True)
                continue

            y_coords, x_coords = indices[:, 0], indices[:, 1]

            # Require a minimum number of points for reliable fitting
            min_points = 15  # Adjusted minimum number of points
            if len(x_coords) < min_points:
                if verbose:
                    print(f"Not enough points ({len(x_coords)}) in bin {intensity_level_percent}% ± {bin_width_percent / 2}% for reliable fitting.", flush=True)
                continue
            ...
            inlier_mask = np.abs(residuals) <= 2 * std_residuals  # Exclude points beyond 2 standard deviations

            # Refit circle with inliers only
            x_inliers = x_coords[inlier_mask]
            y_inliers = y_coords[inlier_mask]

            if len(x_inliers) < min_points:
                if verbose:
                    print(f"Not enough inlier points ({len(x_inliers)}) after outlier removal in bin {intensity_level_percent}% ± {bin_width_percent / 2}%.", flush=True)
                continue
            ...
            if plot:
                ...
                plt.title(f'Intensity bin {intensity_level_percent}% ± {bin_width_percent / 2}%')
                plt.imshow(image * mask, cmap='gray', origin='lower')
                plt.plot(x_coords, y_coords, 'b.', label='All Points in Bin')
                plt.plot(x_inliers, y_inliers, 'g.', label='Inliers')
                circle = plt.Circle((xc, yc), R, color='red', fill=False, linewidth=1, label='Fitted Circle')
                ax = plt.gca()
                ax.add_patch(circle)
                plt.scatter(xc, yc, color='yellow', marker='x', s=1, label='Circle Center')
                plt.legend()
                plt.show()
