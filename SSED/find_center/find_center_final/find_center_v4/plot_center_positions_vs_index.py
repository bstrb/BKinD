...
def plot_center_positions(h5_paths, selected_positions=None, selected_indices=None, labels=None, save_path=None):
    """
    Plots center_x and center_y positions vs positions or indices from multiple HDF5 files.

    Parameters:
        h5_paths (list of str): List of paths to HDF5 files.
        selected_positions (list or array-like, optional): List of positions in the datasets to plot.
            If None, all positions are plotted.
        selected_indices (list or array-like, optional): List of specific image indices to plot. 
    ...
    # Convert selected_positions and selected_indices to numpy arrays for efficient processing
    if selected_positions is not None:
        selected_positions = np.array(selected_positions)
        # Ensure selected_positions are unique and sorted (optional)
    ...
    for idx, h5_file in enumerate(h5_paths):
        ...
        try:
            ...
            num_positions = len(center_x)
            positions = np.arange(num_positions)  # Positions in the dataset

            # If selected_positions is specified, filter the data
            if selected_positions is not None:
                # Create a mask where positions are in selected_positions
                mask = np.isin(positions, selected_positions)
                center_x, center_y, index, positions = center_x[mask], center_y[mask], index[mask], positions[mask]

                if len(positions) == 0:
                    print(f"Warning: No matching selected positions in {h5_file}. Skipping.")
                    continue
            ...
            file_data[label] = (center_x, center_y, index, positions)
            print(f"Successfully read {h5_file} with {len(index)} data points")
        except Exception as e:
    ...
    plt.subplot(2, 1, 1)
    plt.title('Center X Positions vs Position', fontsize=16)

    for label, (center_x, _, _, positions) in file_data.items():
        color = next(color_cycle)
        linestyle = next(linestyle_cycle)
        plt.plot(positions, center_x, label=label, linestyle=linestyle, marker='o', color=color)

    plt.xlabel('Position in Dataset', fontsize=14)
    ...
    plt.subplot(2, 1, 2)
    plt.title('Center Y Positions vs Position', fontsize=16)

    for label, (_, center_y, _, positions) in file_data.items():
        color = next(color_cycle)
        linestyle = next(linestyle_cycle)
        plt.plot(positions, center_y, label=label, linestyle=linestyle, marker='x', color=color)
