    ...
    for root, dirs, files in os.walk(base_path):
        for sub_folder in dirs:
            if sub_folder.startswith("merge"):
                merge_folder_path = os.path.join(root, sub_folder)
                        ...
                        if file.endswith(".txt"):
                            ...
                            if last_cycle_line:
                                ...
                                if numerical_section:
                                    ...
                                    if data is not None:
                                        # Use both the folder two steps above and the "merge" folder name as label
                                        head_folder = os.path.basename(os.path.dirname(root))
                                        label = f"{head_folder}_{sub_folder}"
                                        plot_data(fig, data, label=label)
...
directory = "/home/bubl3932/files/UOX_sim"
