...
if __name__ == "__main__":
    ...
    stream_file_path = '/home/buster/UOX123/3x3_retry/best_exp_results_.stream'  # Update this with your actual file path
    ...
    best_results_sol = read_stream_write_sol(stream_file_path, lattice)
    print(f"Finished processing. Total lines written to .sol file: {best_results_sol}")

    # Generate bash script for fast integration
    bash_file_name = f"exp"
    bash_file_path = os.path.join(stream_file_folder, bash_file_name) + ".sh"
    output_stream_format = f"{stream_file_folder}/{bash_file_name}/int.stream"
