...
def process_image(image, center_x, center_y):
    ...
    drop_threshold = gradient_median - 1 * gradient_std  # Threshold at 2 standard deviations below median

    # Plot gradient for visual verification
    # plt.figure()
    # plt.title('Gradient of Radial Intensity (Reversed)')
    # plt.plot(radial_distances_rev, gradient_rev, 'b-', label='Gradient')
    # plt.axhline(drop_threshold, color='r', linestyle='--', label='Drop Threshold')
    ...
    A_initial = np.max(radial_medians_filtered)*3  # Ensure A_initial is non-negative
    ...
    sigma_initial = (np.max(radial_distances_filtered) - np.min(radial_distances_filtered)) / 20
    gamma_initial = sigma_initial
